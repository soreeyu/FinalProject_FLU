...
.nested_task .gantt_add {
  ...
  var lastUnitNum = 0;
	...
	// offset은 위치가 떨어지는것
	gantt.config.tooltip_offset_x = 100; 
	gantt.config.tooltip_offset_y = 100; 
	...
	//////////////////////////////////lightbox//////////////////////
	 ...
	 var opts = [
		    ...
		    { key:"완료", label: "완료" }                                            
				...
				var full_lightbox =[
				    {name:"unitTitle", height:38, map_to:"text", type:"textarea", focus:true , button:"help"},
				...
				gantt.locale.labels.section_unitTitle = "업무명";
				...
				gantt.attachEvent("onTaskCreated", function(task){
				   //if(task.type == 0){
				    	if (task.type == null) {
				    		...
				    		lastPartNum = lastPartNum +1;
							lastUnitNum = lastUnitNum +1;
							gantt.alert("lastPartNum0= "+lastPartNum+" lastUnitNum0="+lastUnitNum);
				    	}
				    	//안만들어진거라서 안됨
				...
				gantt.attachEvent("onAfterTaskAdd", function(id, item) {
					...
					if (item.type == null) {
						...
						gantt.alert("task"+task);
						gantt.alert("추가했음="+id);
						gantt.alert("task가 처음 만들어졌어요"); //왜 그대가 먼저 실행되는가 //아래부터..
						var newPartId = scheduleNum+"_"+(lastPartNum+1);
						var newUnitId = scheduleNum+"_"+(lastPartNum+1)+"_"+(lastUnitNum+1);
						var taskId = 0;
						gantt.alert("newPartId"+newPartId + "newUnitId" + newUnitId);
						
						
						if(task.$level  == 0){
							//gantt.updateTask(task);
							gantt.changeTaskId(id, newPartId);
							
							
							gantt.alert("part추가한겨");
							//task.id=scheduleNum+"_"+"111";
							gantt.alert(task.id);//왜 이것이 먼저나오냐고후
							curTaskId = newPartId;
							
							var curTask = gantt.getTask(curTaskId);//0인상태
							//curTask.text = "바꿔보쟝";
							curTask.type = 0;
							...
							gantt.changeTaskId(id, newUnitId);
							gantt.alert("unit추가한겨");
							curTaskId = newUnitId;
						...
						gantt.alert("curTaskId " +curTaskId);
				...
				alert("lastPartNum2="+lastPartNum+"  lastUnitNum2="+lastUnitNum);
			...
			alert("lastPartNum="+lastPartNum+"  lastUnitNum"+lastUnitNum);
		</script>
